...
def get_deb_info(deb_file):
    deb_info = {}
    # Use ar command to extract control.tar.gz from the .deb file
    subprocess.run(["ar", "x", deb_file])
    # Extract control file from control.tar.gz
    if os.path.exists("control.tar.gz"):
        with tarfile.open("control.tar.gz") as tar:
            control_file = tar.extractfile("./control").read().decode("utf-8")

        # Split control file into lines and extract necessary information
        for line in control_file.split("\n"):
            if ":" in line:
